...
ll KnapSack(int i, int weight)
{
    ...
    return ret = max(KnapSack(i + 1, weight), a[i].second + KnapSack(i + 1, weight - a[i].first));
}
...
void print_knapsack(int i, int weight)
{
    ...
    if(res == a[i].second + KnapSack(i+1, weight - a[i].first))
    {
        cout << "Take " << i << endl;
        print_knapsack(i+1, weight - a[i].first);
    }else
    {
        cout << "Leave " << i << endl;
